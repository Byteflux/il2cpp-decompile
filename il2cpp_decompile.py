...
def main() -> None:
    ...
    run_il2cpp_dumper([game_assembly_file, global_metadata_file, work_dir])
    run_il2cpp_header_to_ghidra(work_dir)
...
def run_il2cpp_dumper(args: list[str | os.PathLike]) -> None:
    ...
    config_file = il2cpp_dumper_path.parent / "config.json"
    config = {}
    ...
    result = subprocess.run([il2cpp_dumper_path, *args])
    if result.returncode > 0:
        sys.exit(result.returncode)


def run_il2cpp_header_to_ghidra(work_dir) -> None:
...
def run_ghidra(args: list[str | os.PathLike] = []) -> None:
...
def download_and_extract(url: str, name: Optional[str] = None) -> None:
    import requests
